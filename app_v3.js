...
const machineGrid = document.getElementById('machineGridV3');
...
async function initFilters() {
    ...
    dateFilterStart.value = todayStr;
    ...
    dateFilterStart.addEventListener('change', fetchAndRender);
...
function changeDateByOffset(offset) {
    if (!dateFilterStart.value || !dateFilterEnd.value) return;
    const start = new Date(dateFilterStart.value);
    const end = new Date(dateFilterEnd.value);
    start.setDate(start.getDate() + offset);
    end.setDate(end.getDate() + offset);
    dateFilterStart.value = start.toISOString().split('T')[0];
...
async function fetchAndRender() {
    const selectedStart = dateFilterStart.value;
    const selectedEnd = dateFilterEnd.value;
